...
# todo: only works for basic convolutional networks (Linear + Conv2D)
#   - support other layers as well (Attention, BatchNorm, etc)
def load_pytorch_weights_into_network(network, pytorch_model, layer_mapping=None):
    if layer_mapping is None:
        layer_mapping = auto_map_layers(network, pytorch_model) # auto-map
    ...
    for pytorch_id, id in layer_mapping.items():
        ...
        # Load weights across layers
        if isinstance(pytorch_layer, torch.nn.Conv2d) and isinstance(layer, Conv2d):  # conv
            ...
            loaded_count += 1
        elif isinstance(pytorch_layer, torch.nn.Linear) and isinstance(layer, Linear):  # pytorch
...
def auto_map_layers(network, pytorch_network):
